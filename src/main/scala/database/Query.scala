...
object Query {
    ...
    def insertShop(
        ...
        for {
            id  <- sql"""
                    insert into shop (name, business_name, activity_id, stratum_id, address, phone_number, email, website, shop_type_id, position)
                    values ($name, $businessName, $activityId, $stratumId, $address, $phoneNumber, $email, $website, $shopTypeId, ST_MakePoint(${longitude.toFloat}, ${latitude.toFloat}))
